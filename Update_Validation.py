...
class UpdateValidation(ttk.Frame):
    """A frame for validating and updating metadata validation files."""
    def __init__(self, metadata_visualiser):
        super().__init__()
        self.update_command()
        restart_button = tk.Button(self, text="Restart", width=25,
                                   command=lambda:self.restart(), bg='#ff8962')
    ...
    def get_file(self, list_option):
        """Return the file path corresponding to the selected validation list option."""
    ...
    def get_selection(self):
        """Get the selected item from the listbox and populate the update entry field."""
    ...
    def show_options(self):
        """Create and display the validation set selection dropdown and listbox."""
        options_label = tk.Label(
            self, text="Select the validation set to update")
        options_label.grid(column=0, row=3, padx=5, pady=5, sticky='ew')
        choose_validation = tk.StringVar()
        options = ("Collections List", "Digitiser List", "Access List",
                   "Copyright Status List", "Rights Statements List", "Restrictions List")
        ...
        self.drop_menu.grid(column=1, row=3, columnspan=2,
                            sticky='ew', padx=5, pady=5)
        self.drop_menu.bind("<<ComboboxSelected>>", self.show_current_list)
        self.current_list = tk.Listbox(
            self, width=60, height=5, selectmode=tk.SINGLE)
        self.current_list.grid(column=1, row=4, columnspan=2,
                               rowspan=2, sticky='ew', padx=5, pady=5)
    ...
    def update_command(self):
        """Create and display the UI elements for updating validation values."""
        self.show_options()
        ...
        get_selection_button = tk.Button(
            self, text="Get from box", width=25, command=self.get_selection, bg='#c9b8d2')
        update_value_label.grid(column=0, row=7, sticky='e', padx=5, pady=5)
        self.update_value_entry.grid(
            column=1, row=7, sticky='ew', padx=5, pady=5)
        ...
        value_updated_button = tk.Button(self, text="Confirm", width=25, command=lambda: self.update_file(
            self.update_value_entry.get(), self.value_updated_entry.get()), bg='#7fd1ae')
        value_updated_label.grid(column=0, row=8, sticky='e', padx=5, pady=5)
        self.value_updated_entry.grid(
            column=1, row=8, columnspan=2, sticky='ew', padx=5, pady=5)
        value_updated_button.grid(column=2, row=9, sticky='ew', padx=5, pady=5)

    def update_file(self, old, new):
        """Replace an old value with a new value in the validation file."""
    ...
    def restart(self):
        """Clear the entry fields to reset the update form."""
        self.value_updated_entry.delete(0, 'end')
        self.update_value_entry.delete(0, 'end')

    def cb_search(self, event):
        """Filter the listbox contents based on the search string entered."""
    ...
    def fill_listbox(self, item_list, lb):
        """Populate the listbox with items from the provided list."""
        lb.delete(0, 'end')
        for item in item_list:
            lb.insert('end', item)

    def list_box_contents(self, file):
        """Read the contents of a validation file and return as a list of lines."""
        data = []
