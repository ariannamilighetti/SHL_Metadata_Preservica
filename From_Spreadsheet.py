...
class FromSpreadsheet(ttk.Frame):
    """A frame for importing and processing metadata from spreadsheets."""

    def __init__(self):
        super().__init__()
        self.scrollFrame = VerticalScrolledFrame(self)
        self.scrollFrame.pack(fill='both', anchor='center', expand=True)
        self.selection_frm = ttk.LabelFrame(
            self.scrollFrame.viewPort, text="Items")
        self.selection_frm.pack(fill='x', expand=True)
    ...
    def spreadsheeet_field(self):
        """Create and display spreadsheet selection widgets."""
        # Spreadsheet Fields
        template_label = tk.Label(master=self.selection_frm,
                                  text="A template of the correct metadata spreadsheet can be found here:")
        link = tk.Label(self.selection_frm,
                        text="SHL Digitisation Sharepoint > Preservica Prep > Metadata Template for Preservica",
        ...
        link.bind("<Button-1>", lambda e: self.callback(
            "https://uolonline.sharepoint.com/:x:/s/shl/bd/EQqjji8IPAVKgX4y2Qp02fkBLAJq1CnNYHn_0BtrOydGaw?e=Zsh7EL"))
        spreadsheet_label = tk.Label(master=self.selection_frm,
                                     text="Import metadata spreadsheet", anchor='nw')
        spreadsheet_button = tk.Button(self.selection_frm, text='Open',
                                       command=self.upload_spreadsheet, bg='#ff8962')
        self.spreadsheet_filename = tk.Entry(master=self.selection_frm)
        spreadsheet_label.grid(
            column=0, row=2, columnspan=1, sticky="ew", padx=5, pady=5)
        spreadsheet_button.grid(
            column=1, row=2, columnspan=1, sticky="ew", padx=5, pady=5)
        self.spreadsheet_filename.grid(
            column=2, row=2, columnspan=3, sticky="ew", padx=5, pady=5)

    def folder_fields(self):
        """Create and display folder selection widgets."""
        # Folder Fields
        dir_label = tk.Label(master=self.selection_frm,
                             text="Select item(s) parent folder", anchor='nw')
        dir_button = tk.Button(self.selection_frm, text='Open',
                               command=self.ask_directory_ss, bg='#ff8962')
        self.ss_directory_label = tk.Entry(master=self.selection_frm)
        dir_label.grid(column=0, row=3, columnspan=1,
                       sticky="ew", padx=5, pady=5)
        dir_button.grid(column=1, row=3, columnspan=1,
                        sticky="ew", padx=5, pady=5)
        self.ss_directory_label.grid(
            column=2, row=3, columnspan=3, sticky="ew", padx=5, pady=5)

    def check_file_format(self, excel_file):
        '''Check metadata file is in correct format'''
        correct_setup = ['Title', 'Classmark', 'Catalogue number',
                         'Barcode', 'SHL Collection', 'Digitised by',
                         'Digitisation Date', 'Access Conditions',
                         'Restriction reason', 'Restriction expiry date',
                         'Copyright status', 'Copyright Statement', 'Licence details',
                         'Number of Images', 'Digi date text',	'Restr Exp Date']
        validation = True
        i = 0
            ...
            for row in range(1, excel_file.max_row+1):
                ...
                for i in required_columns:
                    if excel_file.cell(row=row, column=i+1).value is None:
                        ...
                        metadata[row -
                                 1].append(excel_file.cell(row=row, column=i+1).value)
        else:
            print("file format incorrect")
        return metadata

    def find_item_metadata(self):
        '''Find the metadata in the metadata file'''
        item_images = []
    ...
    def validate_data(self):
        '''Validate the data in the metadata file'''
        ...
        for key in dict_keys:
            ...
            for i in labels:
                column += 1
                label = tk.Label(
                    outcome_frame,  text=i[0], wraplength=100, background=i[1])
                label.grid(row=row, column=column, padx=10, pady=5)

    def run_button(self):
        '''run the metadata creation'''
        run_button = tk.Button(self.scrollFrame.viewPort, text="Run",
                               command=lambda: self.validate_data(), bg='#7fd1ae')
    ...
    def ask_directory_ss(self, event=None):
        """Open a directory dialog and update the directory label with the selected path."""
    ...
    def complete_label(self, label):
        """Create and display a completion label on the frame."""
