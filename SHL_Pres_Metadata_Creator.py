...
title_lbl = tk.Label(
    master=app, text="SHL Metadata Creator", anchor="w", font=20)
title_lbl.grid(row=0, column=0, sticky="nw")

app_footer = tk.Label(master=app, 
                      text='Created by Arianna Milighetti, Digitisation Coordinator, Senate House Library. Documentation available on SHL Digitisation SharePoint site. Created July 2025.', anchor='w', font=('Arial', 8))
...
instr_lbl = tk.Label(master=content_frame, 
                     text="To create a new metadata xml, choose either 'From Spreadsheet' or 'From Input'.\nTo view the current data validation settings, choose 'Data Validation'.\nTo update the data validation choose 'Update Data Validation'.", anchor="nw", justify='left')
instr_lbl.grid(row=0, column=0, sticky="ew", padx=5)

template_label = tk.Label(master=content_frame,
                          text="Full documentation can be found here:")
link = tk.Label(content_frame, text="SHL Digitisation Sharepoint > Preservica Wiki",
...
link.bind("<Button-1>", lambda e: callback(
    "https://uolonline.sharepoint.com/:u:/r/sites/shl/bd/SitePages/Preservica-Ops.aspx?csf=1&web=1&e=tOpnW7"))

tabs = ttk.Notebook(content_frame)
tabs.grid(row=3, column=0, sticky="ew", padx=5)

# Create Tabs
ssheet = FromSpreadsheet()
