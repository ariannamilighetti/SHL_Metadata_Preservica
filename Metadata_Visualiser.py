...
class Metadata_Visualiser(tk.Frame):
    def __init__(self, master):
        ...
        self.frame.grid_columnconfigure(0, weight=1, minsize=5, pad=1)
    ...
    def refresh(self):
        self.create_list("Digitised by", 'validation_files/digitiser_validation.txt', 3)
        self.create_list("Access Restriction", 'validation_files/access_validation.txt', 5)
        self.create_list("Restriction Type", 'validation_files/restriction_validation.txt', 7)
        self.create_list("Copyright Status", 'validation_files/copyright_status.txt', 9)
        self.create_list("Rights Statement", 'validation_files/rights_statements.txt', 11)
    ...
    def create_collections(self, title, file, column):
        self.coll_lb = tk.Listbox(self.frame, height=30, width=30)
        self.coll_lb.grid(column=column, row=self.list_row, columnspan=3)
    ...
    def create_list(self, title, file, column):
        self.Lb = tk.Listbox(self.frame, height=30, width=25)
...
def __main__():
    ...
    # Create Tabs
    data_input = Metadata_Visualiser(master=tabs)
